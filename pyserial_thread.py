...
#                     ret = segment.split('_')[1].strip()
#                     if ret is not None:
#                         print('RAW QR Multiple', ret)
#                     return segment
#         else:
#             if str(data).lower().startswith('destination_'):
#                 ret = str(data).split('_')[1].strip()
#                 if ret is not None:
#                     print('RAW QR Single', ret)
#                 return data
...
#         command_str = f"TURN_RIGHT {speed}\n"
#     else:
#         command_str = f"MOVE {speed} {speed}\n"  # Adjusted to send speed in both directions
#     arduino.write(command_str.encode())
#     print(f"Sent command: {command_str.strip()}")
...
#             for segment in segments:
#                 if segment.startswith('destination_'):
#                     return segment.split('_')[1].strip()
#         else:
#             if str(data).lower().startswith('destination_'):
#                 return str(data).split('_')[1].strip()
#     except Exception as e:
#         print(f"Error extracting destination: {e}")
#         return None
#     return None
...
#                     time.sleep(30)
#                     # Perform 180-degree turn from the right side
#                     send_command("TURN_RIGHT", speed)
#                     time.sleep(2)  # Adjust the time to complete the 180-degree turn
...
#     # Set speed to 5 if any lines are detected
...
# # Initialize serial communication with Arduino
# arduino = serial.Serial(port='COM5', baudrate=115200, timeout=2)  # Increase timeout
# time.sleep(2)  # Give some time for the serial connection to initialize
...
#     # Set speed to 6 if any lines are detected
#     if left_count > 0 or right_count > 0:
#         speed = 6
...
#                     # Reverse the robot by 180 degrees at speed 10
#                     send_command("REVERSE", 10)
#                     time.sleep(1)  # Adjust the time to achieve 180-degree turn
...
#                     speed = 6  # Set speed to 6
#                     stopped = False  # Resume movement
#                 else:
#                     # Highlight QR code text in green
#                     print(f"Ignored QR code: {s}")
#                     cv2.putText(frame, s, (10, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2, cv2.LINE_AA)
...
#                     # Reverse the robot at speed 6 for 10 seconds
#                     send_command("REVERSE", 6)
#                     time.sleep(10)
...
#                     print("Detected initial QR code, stopping the robot")
#                     qr_detected = True
#                     stopped = True
#                     movement_command = "STOP"
#                     send_command("STOP", 0)
...
#                     print("Robot stopped at initial point")
#                 else:
#                     # Highlight QR code text in green
#                     print(f"Ignored QR code: {s}")
#                     cv2.putText(frame, s, (10, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2, cv2.LINE_AA)
...
#with reverse but after detect moves a head
